        ...
        <div class="main-content-inner">
            ...
            <div class="page-content">
                <div class="page-header">
                    <div class="row">
                        <div class="col-sm-2" >
                            <input id="permissionCode" type="text"  placeholder="权限编号" class="col-xs-3 col-sm-12" />
                        </div>
                        <div class="col-sm-2" >
                            <input id="permissionName" type="text"  placeholder="权限名称" class="col-xs-3 col-sm-12" />
                        </div>
                        <div class="col-sm-1" >
                            <input id="permissionLevel" type="text"  placeholder="菜单级别" class="col-xs-3 col-sm-8" />
                        </div>
                        <div class="col-sm-1" >
                        ...
                        <div class="col-sm-1" >
                            <button id="delBtn" type="button" class="btn btn-info btn-sm">
                                <span class="ace-icon fa fa-remove icon-on-right bigger-110"></span>
                                删除
                            </button>
                        </div>
                        <div class="col-sm-1" >
                            <button id="editBtn" type="button" class="btn btn-info btn-sm">
                                <span class="ace-icon fa fa-search icon-on-right bigger-110"></span>
                                详情
                            </button>
                        </div>
                        <div class="col-sm-1" >
                            <button id="treeBtn" type="button" class="btn btn-info btn-sm">
                                <span class="ace-icon fa fa-search icon-on-right bigger-110"></span>
                                目录树
                            </button>
                        </div>
...
<script type="text/javascript">
    $(function(){
        $("#grid-table").jqGrid({
            ...
            autowidth:true,//自动宽
            colNames:['id','权限类型', '权限编号','权限名称','权限','菜单级别','上级权限编号','图标'],
            colModel:[
                {name:'id',index:'id',sortable:false,hidden:true},
                {name:'permissionTypeDesc',index:'permissionTypeDesc',sortable:false},
                {name:'permissionCode',index:'permissionCode',sortable:false},
                {name:'permissionName',index:'permissionName',sortable:false},
                {name:'permissionValue',index:'permissionValue',sortable:false},
                {name:'permissionLevel',index:'permissionLevel',sortable:false},
                {name:'parentCode',index:'parentCode',sortable:false},
            ...
            ondblClickRow:function (rowid,iRow,iCol,e) {
                //rowid-当前行id,iRow-当前行索引位置,iCol-当前单元格位置索引,e-event 对象
                var data = $('#grid-table').jqGrid("getRowData", rowid);
                window.location.href = "${basePath}/detail/"+data.id;
            },
    ...
    $('#treeBtn').on('click', function() {
        window.location.href = "${basePath}/tree";
    });

    $('#editBtn').on('click', function() {
        if(checkSelect()){
            var grid = $('#grid-table').jqGrid('getGridParam', 'selrow');
            window.location.href = "${basePath}/detail/"+grid;
        }
    });
    ...
    $('#delBtn').on('click', function() {
        if(checkSelect()){
            var grid = $('#grid-table').jqGrid('getGridParam', 'selrow');
            var data = $('#grid-table').jqGrid("getRowData", grid);
            var permissionCode=data.permissionCode;
            bootbox.confirm({
                message: "是否确认删除["+permissionCode+"]?",
                buttons: {
                    confirm: {
                        label: "确认删除",
    ...
    function loadGrid() {
        ...
        $("#grid-table").jqGrid('setGridParam',{datatype:'json',postData: {
            'permissionCode': permissionCode, 'permissionName': permissionName, 'permissionLevel': permissionLevel
        }}).trigger('reloadGrid');
    }
</script>
</body>
